...
there will 49 subsets. Each subset will be a `dataframe` with six variables.
The state variable is not there, of course, because there is only one value per
subset, and so appears once on the data structure.
...
map1 <- expression({
  lapply(seq_along(map.keys), function(r) {
    ...
    outputkey <- line[3]
    outputvalue <- as.data.frame(rbind(line[-3]), stringsAsFactors = FALSE)
    names(outputvalue) <- c(
      "FIPS", "county", "date", 
      "units", "list", "selling"
    )
    inputvalue$units <- as.numeric(inputvalue$units)
    inputvalue$list <- as.numeric(inputvalue$list)
    inputvalue$selling <- as.numeric(inputvalue$selling)
    rhcollect(key, value)
  })
})
```
...
The loop proceeds throught the input keys, the line numbers, as specified by
the first argument of `lapply`. The R `list` object `map.keys` contains them;
it is created by the RHIPE R function that manages the running of the user Map
and Reduce R code. We will show this function later.  The second argument of
the above `lapply' defines the Map function with the
