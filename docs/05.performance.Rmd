...
and V collumns. We will use R function `system.time` to measure elapsed time.
...
from HDFS to the front end is 253.237 seconds which is about 4.2 mins and the whole elapsed time "T" which consists of the 
reading time and the analyzing time is 561.081 seconds, which is less than 9.4 mins. That result is 
...
run to a full cluster performance test. We will show you the basic idea about how to run a multi-factor experiment in next part.
...
reading and analysis within 9.4 mins! Actually, the elapsed time depends on many factors. This presents
