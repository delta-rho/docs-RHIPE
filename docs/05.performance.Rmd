...
introduce `p` here to make the following codes much easier to write and understand.
...
time **O** and we save the value of `t` to the `timing` list. 
...
object `t`. Then we will save `t` and its label **T** as the second component of list `timing`. Till
now, we have finished the single run of the cluster performance test.

### Performance Test Results ###
```{r eval=FALSE, tidy=FALSE}
library(plyr)
df.timing <- ldply(timing, as.data.frame)
df.timing
```
...
```
We can change the structure of `timing` from list to a dataframe and rename it `df.timing`. The data
frame structure can make the analysis much easier than the list structure. The R function `ldply` can
easily finish the converting. Also in your future cluster performance test, you are also recommended
to change your data structure from list to dataframe to do the analysis. The reason we define `timing`
as a list is because for the cluster performance test, you will have a lot of factors to consider, 
like the number of the sample observations, the subset size, the number of vairables and so on, and
`list` structure will make the saving process much easier. And after the performance test, you can 
easily change a list to a dataframe by `ldply`.

As we can see from the `df.timing` dataframe, the elapsed time **O** we need to read the datasets 
