...
```
  rep  n  m v type      t
1   1 25 13 5    O 48.597
```
So `timing` is a data frame with 1 row and 6 collumns. We will save another type of elapsed time **T** into this data frame.
...
### Results of the Single run of the Performance Test ###
```{r eval=FALSE, tidy=FALSE}
timing
```
...
from HDFS to the front end is 48.597 seconds and the whole elapsed time **T** which consists of the 
reading time and the analyzing time is 133.846 seconds, which is less than 2.3 mins. That result is 
impressive! The cluster to run the test here is a very small one with two nodes and 64 GB 
memory in total. You can definitely use RHIPE computation environment to generalize this one sing
run to a full cluster performance test. 

#### A Multi-Factor Experiment for Performance of a Cluster ####

As we can see, `RHIPE` and its computation environment reduce the time of computation dramatically.
In the last example, we are generating $2^25 *2^5 *2^3 = 8GB$ data and we finish the generation part
and analysis part within 2.3 mins! Actually, the elapsed time depends on many factors. This presents
an opportunity for optimizing the computation even further by making the best choice of the factor. 
Our approach to the optimization is to run statistically designed experiments. This experiment consists
of a lot of factors. There are two different types of elapsed time, which means there is one 
computation-type categorical factor. Two statistical factors  are `m` and `v`, which are the kernel 
factors for our experiment. There are also two Hadoop HDFS factors and two Hadoop MapReduce factors,
two hardware factors. In total, we have 9 factors. We also have replicates for each combination of 
these factors. For more details, you can check the paper(link) which describes the whole experiment
process. In this tutorial, we mainly focus on the computation-type category factor `type`and the two 
statistical factors `m` and `p`. We have shown how to run a single run of the performance test, so
we can easily generalize the single run to the whole performance test experiment. We will take the 
...
p.vec   <- 2^v.vec - 1
run.vec <- 3
dir.exp <- "/ln/song273/tmp/multi.factor_n25"
```
...
for (rep in rep.vec) {
  ...
  for (m in m.vec) {
    for (v in v.vec) {
      ...
      mr2
      t      = as.numeric(system.time({rhex(mr2, async=FALSE)})[3])
      t      = data.frame(type=type,n=n,v=v,m=m,rep=rep,t=t)
      timing = rbind(timing,t) 
    }
  }
  ## timing for T
...
The first for-loop stands for the different replicates. The second and third for-loop is for different
combination of `m` and `v`. There is a single run of performance test for any fixed `rep`, `m` and 
`v`. `dir.dm` and `dir.nf` seperately specify the input type and output type for execution function
`mr2`. The `dir.dm` and `dir.gf` seperately specify the input type and output type for execution 
function `mr3`. The `map2`, `map3`, `reduce3` are the map and reduce function we specified before.
...
1         O 25  4  8   1  45.669
2         O 25  5  8   1  72.751
3         O 25  6  8   1 139.356
4         O 25  4  9   1  41.238
5         O 25  5  9   1  67.580
6         O 25  6  9   1 120.831
7         O 25  4 10   1  36.630
8         O 25  5 10   1  65.720
9         O 25  6 10   1 138.821
10        O 25  4 11   1  47.521
11        O 25  5 11   1  83.752
12        O 25  6 11   1 132.721
13        O 25  4 12   1  46.535
14        O 25  5 12   1  78.616
15        O 25  6 12   1 145.310
16        O 25  4 13   1  49.588
17        O 25  5 13   1  84.652
18        O 25  6 13   1 161.892
19        O 25  4 14   1  47.661
20        O 25  5 14   1 103.689
...

```
The `timing` dataframe has 162 rows and 6 collumns because `type` has two levels, `m` has 9 different
values and `v` has 3 different values. For each combination of the factors, we have 3 replicates. So
we have $2*9*3*3=162$ rows. 

#### Visualize the Results ####

1. The first plot describes the relationship between two kinds of elapsed time and subsets size `m`.

[Elapsed time plots against Subsets](./plots/elapse_m.pdf).

As we can see from the first plot, for every value of `v`, the total elapsed time **T** decreases 
first and then increases later as the value of `m` changes. And the three different curves show the
same drop-increase pattern. The following table gives us a summary about the minimum value of **T**
and its corresponding `m` value.

```{r createtable, results='asis', echo=FALSE}
cat("v | m |min(T)/seconds ","--- | --- |---", sep="\n")
x = c("4","13","78.33","5","13","132.99","6","11","263.39")
x = matrix(x,ncol=3,byrow=TRUE)
cat(apply(x,1,function(X) paste(X,collapse=" | ")),sep = "\n")
```


2. The second plot describes the relationship between two kinds of elapsed time and 3 replicates.

[Elapsed time plots against Replicates](./plots/elapsed_rep.pdf).
...
has temperory memory about these files and in the next replicate of reading part, R maybe just reads
these subsets from memory instead of reading them from the HDFS, which will affect our elpased time
experiment.

From the plot2, we can see that when n=25, replication doesn't have the blocking effect.

3. The third plot describes the relationship between two kinds of elapsed time and number of varibales `v`.

[Elapsed time plots against `v`](./plots/elapse_v.pdf).

As we can see from the plot3, for fixed value of `m` , as the value of log2(variables) `v` increases,
the value of log2(elapsed time) increases linearly.
