...
subsets is *R*. As a result, $N = M * R$. We will use R function `system.time` to measure these two
different time which will be introduced in the later session.

In this example, we will show how to do a sing run of the performance test, which means we only pick
one possible value of *M* and one possible value of *P* to see how fast it is to go. 
