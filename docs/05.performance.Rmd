...
the same by default for the sumulating data step. The keys are numeric numbers 1, 2, ...., *R*, 
where *R* is the number of the subsets. In our example, log2(R) = log2(N/M) = 12. They are the 
elements of the list object `map.keys`, which in this case, `map.keys` and `map.values` would be the
same. The output keys from the `map1` function are the same as the input keys. This is defined by the
`map1` function. The output value for each key is a matrix with 2^13 rows and 2^5 columns. As we have
explained before in the `Housing Data` example, the output key-value pairs from the `map` function 
...
of the `input` as Text file input, Sequence file input and lapply input. For `Housing Data` example,
the `input` type is Text file input and in our performance example, the type of `input` is lapply 
input because we are generating numeric data. The argument `input` in our performance test example 
is a vector consisting of two elements. The first element specifies the input key would be from 1 to 
$2^(n-m)$ for `map1` function. Also, it specifies how many tasks you would like Hadoop MapReduce
Framework to run because each task conresponds to one key-value pair. In our performance test example,
each task is to generate a subset with 2^13 rows and 2^5 columns. The second element of the vector 
`input` specifies the numbers of mappers you would like to use. So each mapper would run $2^(n-m)/12$
tasks, where `12` is the value of the second element of `input`. So the value of the second element
would definitely affect the performance test and it is worthy to carry out an experiment to test its
effect. 

Once you try the `Housing Data` example, you have run into most arguments of `rhwatch`. Most of the arguments
of `rhwatch` function have been introduced in the It consists of  several 
...
output file is 170 mb. It would be impractical to read the whole data sets from HDFS to the node
memory. We can only read one file from HDFS by specify the argument `max` of RHIPE function 
...
to read the data from HDFS to the node memory without other operations and record the reading time .
...
List of 2
 ...
 $ :List of 2
  ...
  ..$ t      : num 40.7
```
The elapsed time **O** we need to read the datasets from HDFS to the node memory is 19.6 
