...
Since we run this serial computation in R, we don't need the reading time. That is, we can directly 
analyze the dataset as soon as we generate it. In this case, we only have the total elapsed time T. The R 
object `m.vec` here refers to the number of observations of the datas
