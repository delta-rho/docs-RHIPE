...
log2 number of variables $v = 5$. The size of the dataset is $2^{23}\cdot 2^5 \codt 2^3=2GB$. Since 
...
of the new dataset would be $2^{23}\cdot 2^6 \codt 2^3=4GB$. We can re-run this test.
...
is $2^{25}\cdot 2^5 \codt 2^3=8GB$. The elapsed time `L` is 1815.752 seconds, which is about 30.3 
...
```
So Serial Computation in R has a lot of limitations in large data analysis such as timing costing and
memory limitation. And it is time for us to experience the computation ablity of RHIPE.
...
We contrast the resulsts in the Serial Computation in R and the parallel computation with RHIPE.
