...
2008 to Mar 2014, which is 66 months, for 2883 counties in 48 U.S. states,
excluding Hawaii and Alaska, and also for the District of Columbia which
which  we treat as a state with one county. The data were derived from
224,369 sales of housing units from Quandl's Zillow Housing Data
(www.quandl.com/c/housing).
...
- **date**: measurement month, from 1 to 66
...
serial R. However, we can use them to illustate RHIPE R commands, and how
they are used to analyze data. The small size let's you easily pick up the
data, follow along using the R commands in the tutorial, and explore RHIPE
yourself with other R commands.
...
(https://raw.githubusercontent.com/xiaosutong/docs-RHIPE/gh-pages/housing.txt).
...
You want to read housing.txt into the R instance running on the Hadoop cluster,
form subsets, and then write the subsets as R objects to the HDFS.
Your first step is to copy
housing.txt to the directory on the R-session server where your R
session is running. We will suppose the that the full path name of this
directory is, which we need below is

```{r eval=FALSE, tidy=FALSE}
"/home/myusername/myRdirectory/"
```
Your next step is to get that text file onto the
...
our promise to you is that you never need to use Hadoop commands, so we
provide a RHIPE function, `\rhput()` that will do it for you. 

```{r eval=FALSE, tidy=FALSE}
rhput("/home/myusername/mydirectory/housing.txt", "/ln/tongx/housing/housing.txt")
```
The first argument is the file name on the R session server that is to be
copied. Because the R session is that directory, the name could have been
The
second argument is a pather
