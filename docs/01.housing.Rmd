...
Now we are setting on the frontend of the cluster or any node of the cluster
We've downloaded the text file to our current R working directory:
...
```{r eval=FALSE, tidy=FALSE}
rhput("./housing.txt", "/ln/tmp/housing/housing.txt")
```
...
```{r eval=FALSE, tidy=FALSE}
rhexists("/ln/tmp/housing/housing.txt")
```
```
[1] TRUE
```
