...
In this example, we are assuming that each subset by time is oversize. One way to overcome this
...
reduce5 <- expression(
  ...
  reduce = {
    time500 <- do.call(rbind, reduce.values)
    names(time500) <- c(
      "fips", "county", "state", 
      "units", "list", "selling"
    )
    time500$list <- as.numeric(time500$list)
    time500$selling <- as.numeric(time500$selling)
    rhcollect(reduce.key, time500)
  },
  post = {
  }
)
...
`time500` which is the row-bind of all these 500 rows(recall that each value is a data frame with single 
row from map output) for this key. Then we collected `time500` as value with the key. Then is the next
500 rows for the same key, repeatedly until all rows for this key has been processed. If for one key we
have 1324 values(rows), then finally we will get three key-value pairs with same key by using this reduce
expression.
...
```{r eval=FALSE, tidy=FALSE}
rst <- rhread("/ln/tongx/housing/bytime")
```
```
...
mr6 <- rhwatch(
  map       = map6,
  reduce    = reduce6,
  input     = rhfmt("/ln/tongx/housing/bytime", type = "sequence"),
  output    = rhfmt("/ln/tongx/housing/soldbytime", type = "sequence"),
  mapred    = list(
    mapred.reduce.tasks = 10
  ),
...
mr7 <- rhwatch(
  ...
  reduce    = reduce6,
  input     = rhfmt("/ln/tongx/housing/bytime", type = "sequence"),
  output    = rhfmt("/ln/tongx/housing/soldbytime.combiner", type = "sequence"),
  mapred    = list(
    mapred.reduce.tasks = 10
  ),
...
```{r eval=FALSE, tidy=FALSE}
rst.comb <- rhread("/ln/tongx/housing/soldbytime.combiner")
rst <- rhread("/ln/tongx/housing/soldbytime")
