...
Here are the first few lines of the raw text file:
```
...
- **date**: date in "YYYY-MM-DD" format.  Each month's values are placed at the first day of the month
- **nUnits**: number of units sold
- **listPrice**: monthly median list price dollar per square foot
- **salePrice**: monthly median selling price dollar per square foot
 
The total size of the text file is around 12MB. It is available as 
...
called "/home/tongx" but yours will be different.
...
```{r eval=FALSE, tidy=FALSE}
system("wget http://raw.githubusercontent.com/xiaosutong/docs-RHIPE/gh-pages/housing.txt")
```
...
```{r eval=FALSE, tidy=FALSE}
rhput("./housing.txt", "/user/tongx/housing/housing.txt")
```
...
map1 <- expression({
  lapply(seq_along(map.keys), function(r) {
    ...
    names(value) <- c(
      "fips", "county", "date", 
      "nUnits", "listPrice", "salePrice"
    )
    value$listPrice <- as.numeric(value$listPrice)
    value$salePrice <- as.numeric(value$salePrice)
    rhcollect(key, value)
  })
})
```
...
reduce1 <- expression(
  ...
  reduce = {
    oneState <- rbind(tmp, do.call(rbind, reduce.values))
  },
  post = {
...
map2 <- expression({
  lapply(seq_along(map.keys), function(r) {
    value <- data.frame(
      ...
      listMean = mean(map.values[[r]]$listPrice, na.rm = TRUE),
      saleMean = mean(map.values[[r]]$salePrice, na.rm = TRUE),
...
keys need not be uniue, but in this case we know that there are 49 state names, where no state
is duplicated.  
...
map3 <- expression({
  lapply(seq_along(map.keys), function(r) {
    ...
    names(value) <- c("date", "nUnits", "listPrice", "salePrice")
    value$listPrice <- as.numeric(value$listPrice)
    value$salePrice <- as.numeric(value$salePrice)
    rhcollect(key, value)
  })
...
We've set `mapred.reduce.tasks = 0` because this job omits the optional reduce expression.  
The map contains an infinite loop, so this job will never complete.  But since we've run
it asynchronously, we can check its status with `rhstatus`, which takes as an argument
the object returned by `rhex`.
