...
- **list**: median list price dollar per square foot
- **selling**: median sold price dollar per square foot
...
and .Rhistory is saved. This working directory is locating on the frontend of the node we are sshing.
...
```{r eval=FALSE, tidy=FALSE}
system("wget http://raw.githubusercontent.com/xiaosutong/docs-RHIPE/gh-pages/housing.txt /home/tongx")
```
...
In this function, we need two arguments or two two absolute filepath, one is for 
...
We can see that `housing.txt` file with size 11.82Mb is located under `/ln/tmp/housing/` 
on HDFS.
...
```{r eval=FALSE, tidy=FALSE}
rst <- rhread("/ln/tmp/housing/bystate")
```
```
...
```{r eval=FALSE, tidy=FALSE}
rhls("/ln/tmp/housing/meanbystate")
```
```
  permission owner      group        size          modtime                                     file
...
So there are five files in output "/ln/tmp/housing/meanbystate", named from "part-r-00000" to 
...
```{r eval=FALSE, tidy=FALSE}
rst <- rhread("/ln/tmp/housing/bycounty", max = 10)
```
```
...
```
$names
[1] "time"             "nSold"            "medListPriceSqft" "medSoldPriceSqft"
...
columns: fips, listMean, soldMean, state, and county name. State and county name information 
...
List of 1
 $ :List of 2
  ...
  .. ..$ soldMean: num [1:2883] NaN NaN NaN NaN NaN ...
...
```
```
   fips listMean soldMean state       county
...
mr5 <- rhwatch(
  ...
  reduce   = reduce5,
  input    = rhfmt("/ln/tmp/housing/bycounty", type = "sequence"),
  output   = rhfmt("/ln/tmp/housing/countytostate", type = "sequence"),
  mapred   = list( 
    mapred.reduce.tasks = 10
  ),
  readback = FALSE,
...
```{r eval=FALSE, tidy=FALSE}
rst <- rhread("/ln/tmp/housing/countytostate")
```
```
