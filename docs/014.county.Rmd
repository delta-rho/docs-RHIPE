...
Our division method for the data will be to divide by county,
...
The line number is the key. The value is the data for the line, our case the
7 observations of the 7 variables of the data for one month and one county.
...
The Map output key for each line identifies the county.
`FIPS` would have been enough to do this, but the it is specified as a
character vector with three elements: `FIPS`, `state`, `county`.
...
Reduce assembles the line outputs into groups by the key, which  county, and creates the
