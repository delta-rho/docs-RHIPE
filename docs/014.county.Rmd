...
input key of the reduce step. `reduce.values` is a `list` object which is the input 
values corresponding to the `reduce.key`. Each element is the Reduce input values 
for one Reduce input key. Same as in the Map, `reduce.key` and `reduce.values` are
created by `rhwatch()` when the job is processing.

Reduce groups the Map output key-value pairs by Map output key, the vector of FIPS, 
