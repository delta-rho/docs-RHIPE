...
reduce3 <- expression(
  ...
  post = {
    attr(oneCounty, "location") <- reduce.key[1:3]
    names(attr(oneCounty, "location")) <- c("FIPS","county","state")
...
we initialize an empty `data frame`, `oneCounty`.  In `reduce`,
we use `rbind`
...
pair.  The key is the first element of `reduce.key`, which is the FIPS, and the
value is the `data.frame` 
