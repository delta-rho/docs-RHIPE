...
`input` specifies the input to be the text file in the HDFS that we put there
earlier using `rhput()`. `output` specifies the file name into which final
outputs of the Reduce operation are written. Note that `rhwatch()` creates
this file.
...
You can do this by the argument `readback`. If `TRUE`, the list is also
...
Each one has type `character`.

Next we turn to the Map output key-value pairs.
`outputkey` for react text line is a character vector of length 3 with `FIPS`,
...
line, and writes the results to the HDFS.
...
`rhcollect()` forms a Reduce output key-value pair for each county. RHIPE then
writes the Reduce output key-value pairs to the HDFS.
