...
```
`rhwatch()` creates two list objects `map.keys` and `map.values`.
The first is the Map input keys, which in our case are the text file line
numbers; each element of the list is one line number. The second is the
input values; each element is the observations of the 7 county variables for
one line and consists of one text string. Let `r` be an integer in
 `1:length(map.keys)` then `map.values[[r]]` is the value for key
`map.keys[[r]]`.

This Map code is really a `for loop`, but is done by `lapply()` because it is in
...
The function `strsplit()` splits each line character string of variable
observations into the individual observations of the text line, but each
observation is expressed as a `character` object. `line` is assigned
a `list` of length one whose element is a character vector of length 7.
...
and 4 columns, the observations of `date`, `units`, `list`, and `selling`, now
each as a numeric object.
...
```
`rhwatch()` creates two list objects `reduce.key` and `reduce.value`.
The first is the Map input keys, which in our case are the text file line
numbers; each element of the list is one line number. The second is the
input values; each element is the observations of the 7 county variables for
one line and consists of one text string. Let `r` be an integer in
