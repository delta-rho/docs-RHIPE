...
We can delete, move and copy files on HDFS by using different `RHIPE` functions, and we 
will illustrate how to do them in the following session.

We can use `rhls()` to see that what we've already had in the `/yourloginname/housing` directory:
...
We can delete a directory or a file with the `rhdel()` function:
